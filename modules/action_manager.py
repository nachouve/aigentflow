...
def execute_action(action_name, content, variable_values):
    try:
        ...
        result = subprocess.run(
            modified_content, shell=True, capture_output=True, text=True
        )
    ...
    if st.session_state.execution_result:
        ...
        if st.session_state.execution_returncode == 0:
            ...
            if (
                ...
                and st.session_state.execution_stdout.strip()
            ):
...
def display_action_form(action=None):
    if action:
        ...
        action_content = st.text_area(
            "Action Content (use <variable_name> for variables)",
...
def display_variable_form(variables):
    if "edit_var" in st.session_state and st.session_state.edit_var:
        var_name = st.text_input(
            "Variable Name", value=st.session_state.edit_var["name"]
        )
        var_type = st.selectbox(
            "Variable Type",
            ["text", "number", "date", "options"],
            index=["text", "number", "date", "options"].index(
                st.session_state.edit_var.get("type", "text")
            ),
        )
        if var_type == "options":
            var_options = st.text_area(
            ...
            var_default = st.selectbox(
                ...
                index=0
                if not var_options
                else var_options.split("\n").index(
                    st.session_state.edit_var.get("default", "")
                )
                if st.session_state.edit_var.get("default", "")
                in var_options.split("\n")
                else 0,
            )
        else:
            var_default = st.text_input(
                "Default Value", value=st.session_state.edit_var.get("default", "")
            )
            var_options = []
    else:
    ...
    with col1:
        if st.button("Add/Update Variable", key="add_update_variable"):
            if var_type == "options":
                ...
                variables.append(
                    {"name": var_name, "type": var_type, "default": var_default}
                )
...
def display_actions():
    ...
    if st.session_state.run_action:
        ...
        variable_values = {
            key[4:]: st.session_state[key]
            for key in st.session_state.keys()
            if key.startswith("var_")
        }
        execute_action(
            st.session_state.action_name,
            st.session_state.action_content,
            variable_values,
        )
    ...
    if st.session_state.show_form:
        ...
        st.button(
            "New Action",
            on_click=lambda: setattr(st.session_state, "show_form", True),
            key="new_action",
        )
    ...
    if st.session_state.execute_submit:
        ...
        with st.form(key="execute_action_form"):
            ...
            for var in st.session_state.variables:
                if var["type"] == "text":
                    variable_values[var["name"]] = st.text_input(
                        f"{var['name']}", value=var["default"]
                    )
                elif var["type"] == "number":
                    variable_values[var["name"]] = st.number_input(
                        f"{var['name']}",
                        value=float(var["default"]) if var["default"] else 0,
                    )
                elif var["type"] == "date":
                    ...
                    default_idx = (
                        options.index(var["default"])
                        if var["default"] in options
                        else 0
                    )
                    variable_values[var["name"]] = st.selectbox(
                        f"{var['name']}", options=options, index=default_idx
                    )
