...
def variable_form() -> rx.Component:
    """A form for adding/editing variables for an action."""
    return rx.cond(
        AppState.show_variable_form,
        rx.box(
            rx.vstack(
                rx.heading("Add/Edit Variable", size="4"),
                rx.input(
                    placeholder="Variable Name (e.g., 'filename')",
                    value=ActionState.current_variable_data["name"],
                    on_change=lambda val: ActionState.set_current_variable_data_item(
                        "name", val
                    ),
                    width="100%",
                ),
                ...
                rx.cond(
                    ...
                    rx.text_area(
                        ...
                        value=ActionState.current_variable_data["options"],
                        on_change=lambda val: ActionState.set_current_variable_data_item(
                            "options", val
                        ),
                        width="100%",
                ...
                rx.hstack(
                    rx.button(
                        ...
                        on_click=[
                            ...
                            ActionState.reset_current_variable_data,
                        ],
                        color_scheme="gray",
                        variant="soft",
                    ),
                    rx.button(
                        "Add/Update Variable",
                        on_click=[ActionState.add_variable_to_action],
                    ),
                    spacing="3",
                ),
                spacing="4",
        ...
        rx.form(
            rx.vstack(
                ...
                rx.cond(
                    ...
                    rx.vstack(
                        rx.foreach(
                            ...
                            lambda var_item, index: rx.hstack(
                                ...
                                rx.button(
                                    "Remove",
                                    on_click=lambda: ActionState.remove_variable_from_action(
                                        var_item["name"]
                                    ),
                                    size="1",
                                    color_scheme="red",
                    ...
                    rx.text(
                        "No variables defined for this action yet.", color_scheme="gray"
                    ),
                ),
                rx.button(
                    "Add Variable",
                    on_click=[
                        ...
                        AppState.show_variable_form_dialog,
                    ],
                    margin_top="1em",
                    variant="outline",
                ),
