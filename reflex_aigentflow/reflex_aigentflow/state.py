...
class AppState(rx.State):
    ...
    show_variable_form: bool = False # For managing variables within action/prompt forms
...
class ActionState(AppState):
    ...
    action_name: str = ""
    action_content: str = ""
    action_variables: List[Dict] = [] # As per subtask item 2: "action_variables: list[dict]"
    ...
    action_to_execute_variables: List[Dict] = [] # Storing as list of dicts, can be converted to Variable objects if needed at execution time
    action_variable_values: Dict[str, str] = {} # Stores user-provided values for variables for execution
    ...
    current_variable_data: Dict = {"name": "", "type": "text", "default": "", "options": ""} # options as comma-separated string
    ...
    def prepare_new_action_form(self):
        ...
        self.current_variable_data = {"name": "", "type": "text", "default": "", "options": ""}
    ...
    async def prepare_edit_action_form(self, action_name_to_edit: str):
        async with self:
            ...
            action_to_edit = next((action for action in self.actions if action.get("name") == action_name_to_edit), None)
            if action_to_edit:
                ...
                self.action_variables = [
                    {"name": v.get("name",""), "type": v.get("type","text"), "default": v.get("default",""), "options": v.get("options",[])} 
    ...
    def add_variable_to_action(self):
        ...
        if var_type == "options":
            ...
            new_var["options"] = [opt.strip() for opt in options_str.split(',') if opt.strip()]
        
        self.action_variables.append(new_var)
        self.current_variable_data = {"name": "", "type": "text", "default": "", "options": ""} # Reset form
        self.show_variable_form = False

    def remove_variable_from_action(self, variable_name: str):
        self.action_variables = [v for v in self.action_variables if v.get("name") != variable_name]
    ...
    async def save_action(self):
        async with self:
            ...
            safe_action_name = "".join([c if c.isalnum() else "_" for c in self.action_name.strip()])
            ...
            if self.editing_action_original_name:
                safe_original_name = "".join([c if c.isalnum() else "_" for c in self.editing_action_original_name])
    ...
    def prepare_action_execution(self, action: Dict):
        ...
        self.action_variable_values = {
            ...
            for var in variables_for_execution if var.get("name")
        }
    ...
    async def execute_action(self):
        async with self:
            ...
            for var_name, var_value in self.action_variable_values.items():
                ...
                modified_content = modified_content.replace(f"<{var_name}>", safe_var_value)
            ...
            try:
                ...
                import subprocess
                result = subprocess.run(modified_content, shell=True, capture_output=True, text=True, check=False)
                ...
                self.action_execution_stderr = f"Error: Command not found or path is incorrect. Details: {str(e)}"
                self.action_execution_returncode = e.errno if hasattr(e, 'errno') else -1
                ...
                self.action_execution_stderr = f"An unexpected error occurred during execution: {str(e)}"
                self.action_execution_returncode = -1 # Generic error code for other exceptions
    ...
    async def delete_action(self):
        async with self:
            if self.action_to_delete_name:
                ...
                safe_action_name = "".join([c if c.isalnum() or c in (' ', '_', '-') else '_' for c in self.action_to_delete_name]).replace(' ', '_')
                action_file_to_delete = ACTION_DIR / f"{safe_action_name}.json"

                if action_file_to_delete.exists():
                    action_file_to_delete.unlink()
                else:
                    print(f"Warning: Action file {action_file_to_delete} not found for deletion.")
...
class PromptState(AppState):
    ...
    prompt_name: str = ""
    prompt_content: str = ""
    prompt_variables: List[Dict] = [] # As per subtask item 3: "prompt_variables: list[dict]"
    ...
    prompt_variable_values: Dict[str, str] = {} # Stores user-provided values for variables for execution

    prompt_execution_output: str = "" # As per subtask item 3. For "Show in Window"
    show_prompt_execution_output: bool = False

    # For prompt forms (create/edit) - These are existing
    ...
    async def prepare_edit_prompt_form(self, prompt_name_to_edit: str):
        async with self:
            ...
            prompt_to_edit = next((p for p in self.prompts if p.get("name") == prompt_name_to_edit), None)
            if prompt_to_edit:
                ...
                self.prompt_variables = [
                    {"name": v.get("name",""), "type": v.get("type","text"), "default": v.get("default","")} 
                    for v in loaded_vars
                ]
    ...
    def add_variable_to_prompt(self):
        ...
        if any(v["name"] == var_name for v in self.prompt_variables):
            print(f"Error: Variable '{var_name}' already exists.") # TODO: User-facing error
            return
    ...
    def remove_variable_from_prompt(self, variable_name: str):
        self.prompt_variables = [v for v in self.prompt_variables if v.get("name") != variable_name]
    ...
    async def save_prompt(self):
        async with self:
            ...
            safe_prompt_name = "".join([c if c.isalnum() else "_" for c in self.prompt_name.strip()])
            filename = PROMPT_DIR / f"{safe_prompt_name}.json"

            if self.editing_prompt_original_name:
                safe_original_name = "".join([c if c.isalnum() else "_" for c in self.editing_prompt_original_name])
    ...
    def execute_prompt_use_as_input(self):
        ...
        print(f"Attempting to use prompt output for action: {self.selected_action_for_prompt_input}, variable: {self.selected_target_variable_for_prompt_input}")
    ...
    def execute_prompt_show_in_window(self):
        if self.prompt_to_execute_content is None:
            ...
            for var_name, var_value in self.prompt_variable_values.items():
                str_var_value = str(var_value) if var_value is not None else ""
                modified_content = modified_content.replace(f"<{var_name}>", str_var_value)
    ...
    async def delete_prompt(self):
        async with self:
            if self.prompt_to_delete_name:
                ...
                safe_prompt_name = "".join([c if c.isalnum() or c in (' ', '_', '-') else '_' for c in self.prompt_to_delete_name]).replace(' ', '_')
                prompt_file_to_delete = PROMPT_DIR / f"{safe_prompt_name}.json"
                if prompt_file_to_delete.exists():
                    prompt_file_to_delete.unlink()
                else:
                    print(f"Warning: Prompt file {prompt_file_to_delete} not found for deletion.")
...
class HistoryState(AppState):
    ...
    async def load_history(self):
        async with self:
            ...
            history_files = sorted(HISTORY_DIR.glob("*.json"), key=lambda f: f.name, reverse=True)
            ...
            for history_file in history_files:
                with open(history_file, "r") as file:
                    try:
                        ...
                        self.history_record_files.append(history_file.name) # Store filename
                    except json.JSONDecodeError:
                        print(f"Warning: Could not decode JSON from history file {history_file}")
