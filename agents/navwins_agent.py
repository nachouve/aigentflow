...
class WindowManager(ABC):
    ...
    def _matches_criteria(self, window_title, title_name, title_must_contain, ignore_case):
        window_title = window_title.upper() if ignore_case else window_title
        if title_name and (title_name.upper() if ignore_case else title_name) in window_title:
            return True
        if title_must_contain and all((title.upper() if ignore_case else title) in window_title for title in title_must_contain):
            return True
        return False
...
class WindowsManager(WindowManager):
    ...
    def activate_window(self, title_name=None, title_must_contain=[], ignore_case=True):
        for window in self.gw.getAllWindows():
            if self._matches_criteria(window.title, title_name, title_must_contain, ignore_case):
                window.activate()
                return True, window
        return False, None
...
class LinuxManager(WindowManager):
    ...
    def activate_window(self, title_name=None, title_must_contain=[], ignore_case=True):
        for window in self.ewmh.getClientList():
            if self._matches_criteria(window, title_name, title_must_contain, ignore_case):
...
class MacManager(WindowManager):
    ...
    def list_windows(self):
        return self.appscript.app('System Events').processes.whose(visible=True).name.get()
...
def go_down_and_click(window, down_num=20, pixel_from_bottom=100):
    ...
    right_bottom = window.left + window.width // 2, window.top + window.height - pixel_from_bottom
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--debug":
            ...
            strings_to_find = input("Enter the strings to find separated by comma: ").split(",")
    else:
